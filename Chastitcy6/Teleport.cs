...
namespace Chastitcy6
{
    ...
    public class Teleport : IImpactPoint
    {
        ...
        {
            // считаем смещение частицы от центра «входа» портала
            float dx = p.X - X;
            float dy = p.Y - Y;

            // если квадрат расстояния ≤ квадрату радиуса — значит внутри круга
            if (dx * dx + dy * dy <= Radius * Radius)
            {
                // телепортируем: меняем координаты частицы на точку выхода
                p.X = ExitX;
                p.Y = ExitY;
            }
        }
